...
export const Header: React.FC<HeaderProps> = ({ onOpenInstallModal }) => {
  ...
  return (
    <header id="milenia-header" className="sticky top-0 z-40 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 shadow-xs transition-colors duration-300 w-full max-w-full overflow-x-clip">
      
      {/* Top Banner: Location info & Fulfillment mode & Firebase Status */}
      <div className="bg-slate-900 dark:bg-black text-slate-200 text-xs border-b border-slate-800 dark:border-slate-900 w-full">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-1.5 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 hover:text-amber-400 font-medium transition cursor-pointer text-xs"
                id="header-location-selector-btn"
              >
                <MapPin className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                <span className="truncate max-w-[130px] sm:max-w-[200px]">{selectedLocation.name}</span>
                <span className="text-emerald-400 text-[10px] font-semibold bg-emerald-950/80 px-1.5 py-0.5 rounded-full border border-emerald-800 shrink-0">
            ...
            {/* Opening times note */}
            <div className="hidden md:flex items-center gap-1 text-slate-400 text-xs shrink-0">
            ...
            <div className="hidden xl:flex items-center gap-1 text-[11px] text-slate-400 bg-slate-800/90 dark:bg-slate-900 px-2 py-0.5 rounded-full border border-slate-700 shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1 text-slate-300 hover:text-white px-2 py-1 rounded-full hover:bg-slate-800 transition cursor-pointer text-xs"
              id="lang-toggle-btn"
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full text-xs font-semibold transition cursor-pointer ${
                currentView === 'admin' 
            ...
            >
              ...
              <span className="hidden sm:inline">{currentView === 'admin' ? (language === 'es' ? '← Carta' : '← Menu') : (language === 'es' ? 'KDS Staff' : 'KDS Staff')}</span>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 w-full">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-6 shrink-0">
            ...
            <div className="hidden xl:flex items-center bg-slate-100 dark:bg-slate-900 p-1 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs">
              <button
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 xl:px-3.5 py-1.5 xl:py-2 rounded-xl text-xs xl:text-sm font-medium transition cursor-pointer relative whitespace-nowrap ${
                    isActive 
          ...
          <div className="flex items-center gap-2 shrink-0">
            {!isInstalled && (
              <button
                ...
                className="hidden 2xl:flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30 transition cursor-pointer"
            ...
            {/* Milenia Rewards Club Button */}
            <button
              onClick={() => setIsRewardsOpen(true)}
              id="header-rewards-btn"
              className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-2xl text-xs font-bold bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/30 transition cursor-pointer shrink-0"
              title="Milenia Rewards Club"
            >
              <Crown className="w-4 h-4 text-amber-500 shrink-0" />
              <div className="text-left leading-none">
                <div className="text-[9px] uppercase font-bold tracking-wider text-amber-600/80 dark:text-amber-400/80">Rewards</div>
                <div className="font-black text-slate-900 dark:text-amber-300">{rewardsProfile.currentPoints} pts</div>
              </div>
            </button>
            ...
            <button
              ...
              className="relative flex items-center gap-2 bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl font-black text-xs sm:text-sm shadow-md hover:shadow-lg shadow-amber-500/20 transition-all cursor-pointer shrink-0"
            >
              ...
              <div className="flex flex-col items-start leading-none text-left">
                <span className="text-[9px] sm:text-[10px] text-slate-900/80 uppercase tracking-wider font-bold">{language === 'es' ? 'Mi Cesta' : 'Cart'}</span>
                <span className="font-black text-slate-950 text-xs sm:text-sm">€{subtotal.toFixed(2)}</span>
              </div>
            </button>
        ...
        {isMobileMenuOpen && (
          <div id="mobile-nav-drawer" className="lg:hidden border-t border-slate-200 dark:border-slate-800 py-3 pb-5 animate-in slide-in-from-top-2">
            {/* Mobile Order Type */}
